...
BOOTPARTNAME_cubieboard = "CUBIEBOARD"
...
IMAGE_CMD_sdimg () {
	...
	dd if=/dev/zero of=${LOOPDEV} bs=1M count=1
	...
	if [ -e ${IMAGE_ROOTFS}/boot/script.bin ] ; then
		cp -v ${IMAGE_ROOTFS}/boot/script.bin ${WORKDIR}/tmp-mnt-boot 
	else
		cp -v ${DEPLOY_DIR_IMAGE}/script.bin ${WORKDIR}/tmp-mnt-boot
	fi
